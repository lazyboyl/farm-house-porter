	...
	<view>
		...
		<view class="goods-section">
			<view class="g-header b-b">
				...
				<text class="name">农家搬运工的小店</text>
			</view>
			<!-- 商品列表 -->
			<view class="g-item" v-for="(item, index) in orderInfo.cartVoList" :key="item.cartId">
				<image :src="'http://127.0.0.1/fhp' + item.image"></image>
				<view class="right">
					<text class="title clamp">{{item.title}}</text>
					<text class="spec">{{item.attrVal}}</text>
		...
		<view class="yt-list">
			<view class="yt-list-cell b-b">
				...
				<text class="cell-tip">￥{{orderInfo.totalPrice + orderInfo.discountPrice}}</text>
			</view>
			<view class="yt-list-cell b-b">
				<text class="cell-tit clamp">优惠金额</text>
				<text class="cell-tip red">-￥{{orderInfo.discountPrice}}</text>
			</view>
			<view class="yt-list-cell b-b">
...
<script>
	import {getCreateOrder} from "../../api/order/api.order.js";
	import {getDefaultAddress} from "../../api/address/api.address.js";
	import {submitOrder} from "../../api/order/api.order.js";
	...
	export default {
		...
		onLoad(option){
			this.loadOrder(option.orderId);
			this.loadDefaulrAddress();
		},
		methods: {
			loadDefaulrAddress(){
				let _this = this;
			...
			loadOrder(orderId){
				let _this = this;
				getCreateOrder({orderId:orderId}).then(res=>{
					if(res.code == 200){
						_this.orderInfo = res.obj;
					}
